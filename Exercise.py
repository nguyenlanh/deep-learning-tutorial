...
def backprop(Yi, Xi, W1, b1, W2, b2): 
  ...
  temp2 = scale(2 * diff, dh3)
  b2 = temp2
  temp2 = scale(temp2[0], h2)
  N = [];
  N.append(temp2) 
 
  M = scale(2 * diff * dh3[0], W2[0])
  M = ew_dot(M, dh2)  
  b1 = M
  M = cross(M, Xi) 
...
def sgd(X, Y, alpha):
  ...
  for i in range(10000):
    ...
    W1grad = scale_matrix(-alpha, W1grad)
    b1grad = scale(-alpha, b1grad)
    W2grad = scale_matrix(-alpha, W2grad)
    b2grad = scale(-alpha, b2grad)
         
    ###########################################
    # For now, I just keep the parameters to be the same. Replace the
    # following 4 lines with your own updates.
    W1 = add_matrix(W1, W1grad)
    W2 = add_matrix(W2, W2grad)
    b1 = add(b1, b1grad)  
    b2 = add(b2, b2grad)
